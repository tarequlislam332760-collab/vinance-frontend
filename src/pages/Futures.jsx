...
import {
  ...
  FileText, BarChart2, X, ExternalLink, Link,
  AlertTriangle, Info, Sliders, Layout
} from 'lucide-react';
...
const FuturesStyles = () => (
  <style>{`
    ...
    .fut-pos-tabs{display:flex;padding:0 12px;border-bottom:1px solid #1e2329;overflow-x:auto;scrollbar-width:none;flex-shrink:0;}
    .fut-pos-tabs::-webkit-scrollbar{display:none;}
    .fut-pos-tab{padding:10px 12px 8px;font-size:11px;font-weight:600;background:transparent;border:none;border-bottom:2px solid transparent;color:#5e6673;cursor:pointer;white-space:nowrap;font-family:inherit;transition:all .15s;}
    ...
    .bottom-ticker{display:flex;padding:5px 12px;background:#0b0e11;border-top:1px solid #1e2329;overflow:hidden;font-size:10px;white-space:nowrap;}
    ...
    .notif-badge{position:absolute;top:-3px;right:-3px;width:15px;height:15px;background:#f6465d;border-radius:50%;font-size:8px;font-weight:700;color:#fff;display:flex;align-items:center;justify-content:center;border:2px solid #0b0e11;}
    /* Dropdown */
    ...
    .threedot-item{display:flex;align-items:center;gap:10px;padding:10px 14px;font-size:12px;color:#848e9c;cursor:pointer;font-family:inherit;transition:all .15s;}
    ...
    .settings-nav-item{padding:10px 14px;font-size:12px;cursor:pointer;display:flex;align-items:center;gap:8px;color:#848e9c;font-family:inherit;transition:all .15s;}
    .settings-nav-item:hover{background:#2b3139;color:#eaecef;}
    .settings-nav-item.active{background:#2b3139;color:#f0b90b;}
    .settings-content{flex:1;padding:16px;overflow-y:auto;}
    .settings-row{display:flex;align-items:center;justify-content:space-between;padding:9px 0;border-bottom:1px solid #1e2329;}
    ...
    .s-section-label{color:#5e6673;font-size:10px;margin:12px 0 8px;text-transform:uppercase;letter-spacing:.05em;}
...
const NotifDropdown = ({ onClose, onReadAll }) => {
  ...
  const deleteOne = (id, e) => { e.stopPropagation(); setNotifs(p => p.filter(n => n.id!==id)); };
  ...
  return (
    <>
      ...
      <div className="notif-drop">
        ...
        <div style={{ maxHeight:300, overflowY:'auto', scrollbarWidth:'thin', scrollbarColor:'#2b3139 transparent' }}>
          {list.length === 0 ? (
            ...
            <div key={n.id} className={`notif-item${!n.read?' unread':''}`} onClick={() => markOne(n.id)}>
              <div style={{ width:32, height:32, borderRadius:'50%', background:`${n.color}18`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:15 }}>{n.icon}</div>
              ...
              <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4, flexShrink:0 }}>
                ...
                <button onClick={(e) => deleteOne(n.id, e)} style={{ background:'none', border:'none', color:'#5e6673', cursor:'pointer', padding:2 }}><X size={9}/></button>
              </div>
            </div>
          ))}
        </div>
...
const SettingsModal = ({ onClose }) => {
  ...
  const Toggle = ({ k }) => (
    <button className={`s-toggle${cfg[k]?' on':''}`} onClick={() => tog(k)}>
      ...
      <div className="settings-modal">
        ...
        <div className="settings-body">
          <div className="settings-nav">
            {navItems.map(n => (
              <div key={n.k} className={`settings-nav-item${tab===n.k?' active':''}`} onClick={() => setTab(n.k)}>
                {n.icon}{n.label}
              </div>
            ))}
          </div>
          <div className="settings-content">
            ...
            {tab==='trading' && <>
              <p className="s-section-label">Order Settings</p>
              {[['confirmOrders','Confirm orders before placing'],['autoClose','Auto-close at liquidation price']].map(([k,l]) => (
                <div key={k} className="settings-row"><span className="s-label">{l}</span><Toggle k={k}/></div>
              ))}
              ...
              <p className="s-section-label">Sound</p>
              {[['soundAlerts','Sound alerts on order fill'],['priceAlerts','Price movement alerts']].map(([k,l]) => (
                <div key={k} className="settings-row"><span className="s-label">{l}</span><Toggle k={k}/></div>
              ))}
            </>}

            {tab==='display' && <>
              <p className="s-section-label">Chart & Colors</p>
              <div className="settings-row">
            ...
            {tab==='alerts' && <>
              <p className="s-section-label">Push Notifications</p>
            ...
            {tab==='security' && <>
              <p className="s-section-label">Account Security</p>
...
const ThreeDotMenu = ({ coin, onClose }) => {
  const items = [
    ...
    { icon:<Link size={12}/>,          label:'Copy Chart Link',    fn:() => { navigator.clipboard?.writeText(window.location.href); toast.success('Link copied!'); onClose(); } },
    { icon:<BarChart2 size={12}/>,     label:'Trading Data',      fn:() => { toast('Trading data panel'); onClose(); } },
    { icon:<AlertTriangle size={12}/>, label:'Set Price Alert',   fn:() => { toast('Price alert: coming soon'); onClose(); } },
    { divider:true },
    { icon:<Info size={12}/>,          label:'Market Info',       fn:() => { toast('Market info panel'); onClose(); } },
    ...
    <>
      ...
      <div className="threedot-drop">
        {items.map((item,i) => item.divider
          ...
          : <div key={i} className="threedot-item" onClick={item.fn}>
...
const FuturesOrderBook = ({ symbol, currentPrice, priceUp }) => {
  ...
  useEffect(() => {
    ...
    wtRef.current.onmessage = e => {
      const d = JSON.parse(e.data);
      setTrades(p => [{ price:parseFloat(d.p).toFixed(2), qty:parseFloat(d.q).toFixed(3), time:new Date(d.T).toTimeString().slice(0,8), isBuy:!d.m }, ...p].slice(0,30));
    };
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', gap:4 }}>
          {['book','trades'].map(v => (
            <button key={v} onClick={() => setActiveView(v)}
              style={{ background:activeView===v?'#2b3139':'transparent', border:'none', borderRadius:3, padding:'3px 8px', color:activeView===v?'#eaecef':'#5e6673', cursor:'pointer', fontSize:10, fontWeight:600, fontFamily:'inherit' }}>
              {v==='book'?'Book':'Trades'}
            </button>
          ))}
        </div>
      ...
      {activeView==='book' ? <>
        <div className="fut-ob-header"><span>Price (USDT)</span><span>Size</span><span>Sum</span></div>
...
const HistoryTable = ({ type, token }) => {
  ...
  if (!filtered.length) return <div className="empty-state"><FileText size={28} style={{opacity:.2}}/><span>No history found</span></div>;
  return (
    <div style={{ overflowX:'auto' }}>
      <table style={{ width:'100%', fontSize:11, borderCollapse:'collapse' }}>
        <thead>
          <tr style={{ color:'#5e6673', borderBottom:'1px solid #1e2329' }}>
            {['Date','Type','Amount','Status'].map(h => <th key={h} style={{ padding:'6px 10px', textAlign:'left', fontWeight:600, textTransform:'uppercase', fontSize:10 }}>{h}</th>)}
          </tr>
        </thead>
        <tbody>
          {filtered.map((t,i) => (
            <tr key={i} style={{ borderBottom:'1px solid #1e232960' }}>
              ...
              <td style={{ padding:'7px 10px', color:t.type==='deposit'?'#0ecb81':t.type==='withdraw'?'#f6465d':'#f0b90b', textTransform:'uppercase', fontWeight:700 }}>{(t.type||'').replace('_',' ')}</td>
...
const Futures = () => {
  ...
  useEffect(() => {
    ...
    ws.onmessage = e => {
      ...
      setTicker24h(p => ({ ...p, change:parseFloat(d.p).toFixed(2), changePct:parseFloat(d.P).toFixed(2), high:parseFloat(d.h).toFixed(2), low:parseFloat(d.l).toFixed(2), vol:parseFloat(d.v).toLocaleString(undefined,{maximumFractionDigits:0}), markPrice:(price+Math.random()*.5-.25).toFixed(2) }));
    };
    ws.onerror = () => ws.close();
    return () => ws.close();
  }, [coin]);
  ...
  const handleTrade = async () => {
    ...
    try {
      const res = await api.post('/api/futures/trade', {
        type:side, amount:parseFloat(amount), leverage:Number(leverage), symbol:coin, entryPrice:parseFloat(currentPrice),
    ...
    <>
      ...
      <div className="fut-wrap">
        ...
        <div className="fut-header">
          ...
          {/* Stats */}
          <div style={{ display:'flex', gap:20, overflow:'hidden', flex:1, justifyContent:'center' }}>
          ...
          <div className="fut-center">
            ...
            <div style={{ height:260, display:'flex', flexDirection:'column', borderTop:'1px solid #1e2329', background:'#0b0e11' }}>
              <div className="fut-pos-tabs">
                {[{key:'positions',label:`Positions(${positions.length})`},{key:'open_orders',label:'Open Orders(0)'},{key:'order_history',label:'Order History'},{key:'trade_history',label:'Trade History'},{key:'transaction',label:'Transaction History'},{key:'position_history',label:'Position History'},{key:'assets',label:'Assets'}].map(t => (
                ...
                <button onClick={fetchPositions} style={{ marginLeft:'auto', background:'none', border:'none', cursor:'pointer', color:'#5e6673', padding:'0 8px' }}>
              ...
              <div style={{ flex:1, overflowY:'auto', scrollbarWidth:'thin', scrollbarColor:'#2b3139 transparent' }}>
                ...
                {activePosTab==='open_orders' && <div className="empty-state"><FileText size={28} style={{opacity:.2}}/><span>No open orders.</span></div>}
                {(activePosTab==='order_history'||activePosTab==='trade_history') && <HistoryTable type="trade" token={token}/>}
                {activePosTab==='transaction' && <HistoryTable type="all" token={token}/>}
                {activePosTab==='position_history' && <HistoryTable type="trade" token={token}/>}
                {activePosTab==='assets' && (
                  <div style={{ padding:'12px 16px', fontSize:12 }}>
                    {[['Wallet Balance',`${(user?.balance||0).toFixed(4)} USDT`],['Margin Balance',`${(user?.balance||0).toFixed(4)} USDT`],['Unrealized PNL','0.00 USDT'],['Maintenance Margin','0.0000 USDT']].map(([l,v]) => (
          ...
          <div className="fut-right">
            ...
            <div style={{ padding:'12px', display:'flex', flexDirection:'column', gap:10, flex:1, overflowY:'auto' }}>
              ...
              <div>
                ...
                <div className="leverage-track" onClick={e => {
                  ...
                  setLeverage(Math.round(Math.max(1,Math.min(125,((e.clientX-r.left)/r.width)*125))));
                }}>
              ...
              <button onClick={handleTrade} disabled={loading}
                style={{ width:'100%', padding:'13px 0', border:'none', borderRadius:6, fontSize:14, fontWeight:700, cursor:loading?'not-allowed':'pointer',
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, fontSize:10 }}>
                <div><div style={{color:'#5e6673'}}>Liq Price</div><div style={{color:'#eaecef'}}>-- USDT</div></div>
                <div style={{textAlign:'right'}}><div style={{color:'#5e6673'}}>Liq Price</div><div style={{color:'#eaecef'}}>-- USDT</div></div>
              </div>
            </div>
            ...
            <div className="acct-panel">
              ...
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
                <div style={{ width:28, height:28, borderRadius:'50%', border:'3px solid #0ecb81', display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, color:'#0ecb81', fontWeight:700 }}>⊙</div>
                <span style={{ color:'#0ecb81', fontSize:13, fontWeight:700 }}>0.00%</span>
              </div>
              {[['Margin Ratio','0.00%'],['Maintenance Margin','0.0000 USDT'],['Margin Balance',`${(user?.balance||0).toFixed(4)} USDT`]].map(([l,v]) => (
                <div key={l} className="acct-row"><span style={{color:'#5e6673'}}>{l}</span><span style={{color:'#eaecef',fontWeight:600}}>{v}</span></div>
              ))}
              <button style={{ width:'100%', marginTop:10, padding:'7px 0', background:'#2b3139', border:'none', borderRadius:4, color:'#eaecef', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>Single-Asset Mode</button>
              <div style={{ display:'flex', gap:6, marginTop:8 }}>
                {['Transfer','Buy Crypto','Swap'].map(b => (
                  <button key={b} style={{ flex:1, padding:'6px 0', background:'#2b3139', border:'none', borderRadius:4, color:'#eaecef', fontSize:10, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>{b}</button>
                ))}
              </div>
            </div>
          </div>
