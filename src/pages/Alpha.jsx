...
import {
  Zap, Search, Star, RefreshCw, TrendingUp, ExternalLink,
  ChevronLeft, AlertCircle, ArrowUpRight, BarChart2,
...
const css = `
  ...
  .al-btn.g{background:rgba(14,203,129,.1);color:#0ecb81;border:1px solid rgba(14,203,129,.25);}
  ...
  @media(max-width:768px){
    .token-row{grid-template-columns:2fr 1fr 1fr;}.hide-m{display:none!important;}
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            <div style={{ display:'flex', alignItems:'center', gap:7, background:'#161a1e', border:'1px solid #2b3139', borderRadius:8, padding:'7px 12px' }}>
              ...
              <input type="text" placeholder="Search tokens..." value={search} onChange={e => setSearch(e.target.value)}
                style={{ background:'transparent', border:'none', outline:'none', color:'#eaecef', fontSize:13, width:140, fontFamily:'inherit' }}/>
              {search && <button onClick={() => setSearch('')} style={{ background:'none', border:'none', color:'#848e9c', cursor:'pointer', padding:0, display:'flex' }}>✕</button>}
            </div>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'20px' }}>
              ...
              <div key={token.symbol} className="token-row fade">
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  <div style={{ minWidth:0 }}>
                    ...
                    <div style={{ fontSize:11, color:'#848e9c', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{token.name} · {token.chain}</div>
                  </div>
                </div>
                {/* Price */}
          ...
          <div style={{ marginTop:20, padding:'12px 16px', background:'rgba(246,70,93,.05)', border:'1px solid rgba(246,70,93,.2)', borderRadius:10, display:'flex', gap:10, alignItems:'flex-start' }}>
            ...
            <p style={{ fontSize:11, color:'#848e9c', lineHeight:1.6 }}>
              <strong style={{ color:'#f6465d' }}>High Risk Warning:</strong> Alpha tokens are early-stage and extremely volatile. Always DYOR before investing. These tokens may lose all value. Never invest more than you can afford to lose.
            </p>
          </div>
        </div>
      </div>
