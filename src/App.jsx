...
// --- Futures Component (Fully Responsive) ---
const Futures = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0b0e11] text-[#eaecef] overflow-hidden font-sans">
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col border-b lg:border-b-0 lg:border-r border-gray-900 min-h-[40vh] lg:min-h-full">
          <div className="flex justify-between items-center px-4 py-2 text-[10px] text-gray-400 border-b border-gray-900 overflow-x-auto no-scrollbar">
            <div className="flex gap-4 whitespace-nowrap">
               <span className="text-[#f0b90b] font-bold">Time</span>
               {['15', '60', '240', 'D'].map(tf => (
                 <span key={tf} onClick={() => setTimeframe(tf)} className={`cursor-pointer hover:text-white ${timeframe === tf ? 'text-white font-bold underline underline-offset-4' : ''}`}>
        ...
        {/* Order Panel - Responsive Layout */}
        <div className="w-full lg:w-[350px] flex flex-row lg:flex-col overflow-y-auto bg-[#0b0e11] pb-24 lg:pb-0">
          <div className="w-[55%] lg:w-full p-4 space-y-4 border-r lg:border-r-0 border-gray-900">
            <div className="flex gap-2">
              <button className="flex-1 bg-[#2b3139] py-2 rounded text-[10px] font-bold uppercase tracking-wider">Cross</button>
              <button className="flex-1 bg-[#2b3139] py-2 rounded text-[10px] font-bold uppercase tracking-wider">{leverage}x</button>
            </div>
            <div className="flex bg-[#2b3139] rounded-lg overflow-hidden p-1">
              <button onClick={() => setSide('buy')} className={`flex-1 py-2 rounded-md text-[11px] font-black uppercase transition-all ${side === 'buy' ? 'bg-[#02c076] text-[#0b0e11]' : 'text-gray-400'}`}>Buy</button>
              <button onClick={() => setSide('sell')} className={`flex-1 py-2 rounded-md text-[11px] font-black uppercase transition-all ${side === 'sell' ? 'bg-[#f6465d] text-white' : 'text-gray-400'}`}>Sell</button>
            </div>
            <div className="relative">
              <input type="number" placeholder="Amount" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full bg-[#2b3139] rounded-lg p-3 text-xs font-bold outline-none text-center border border-transparent focus:border-[#f0b90b]" />
              <span className="absolute right-3 top-3 text-[10px] text-gray-500 font-bold">USDT</span>
            </div>
            <button onClick={handleTrade} className={`w-full py-3.5 rounded-lg font-black text-xs uppercase shadow-lg transition-all active:scale-95 ${side === 'buy' ? 'bg-[#02c076] text-[#0b0e11]' : 'bg-[#f6465d] text-white'}`}>
              {loading ? "Processing..." : side === 'buy' ? "Open Long" : "Open Short"}
            </button>
          </div>
          
          {/* Order Book Side */}
          <div className="w-[45%] lg:w-full p-3 text-[9px] flex flex-col justify-center lg:justify-start">
            <div className="flex justify-between text-gray-500 mb-2 px-1 font-bold"><span>Price</span><span>Qty</span></div>
            <div className="space-y-1.5">
               <div className="flex justify-between px-1"><span className="text-[#f6465d] font-bold">67845.5</span><span className="text-gray-400">1.2k</span></div>
               <div className="flex justify-between px-1"><span className="text-[#f6465d] font-bold">67840.1</span><span className="text-gray-400">0.5k</span></div>
               <div className="text-center py-2 border-y border-gray-900 my-1"><div className="text-[#02c076] text-sm font-black">67,830.9</div></div>
               <div className="flex justify-between px-1"><span className="text-[#02c076] font-bold">67828.2</span><span className="text-gray-400">850</span></div>
               <div className="flex justify-between px-1"><span className="text-[#02c076] font-bold">67825.0</span><span className="text-gray-400">2.1k</span></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#0b0e11] text-white flex flex-col md:flex-row overflow-hidden font-sans">
      ...
      {token && !isHomePage && (
        <aside className="w-20 lg:w-64 bg-[#161a1e] border-r border-[#1e2329] hidden md:flex flex-col p-4 h-screen sticky top-0 z-40">
          ...
          <button onClick={logout} className="p-4 text-gray-500 hover:text-[#f6465d] flex items-center gap-4 font-bold border-t border-[#1e2329] mt-4">
            <LogOut size={20}/> <span className="hidden lg:inline text-[10px] font-black uppercase">Logout</span>
          </button>
        </aside>
      )}
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        {/* Header - Only visible on authenticated pages */}
        {token && !isHomePage && (
          <header className="h-14 border-b border-[#1e2329] bg-[#161a1e] flex items-center justify-between px-6 sticky top-0 z-30">
            <div className="font-black text-[10px] uppercase tracking-widest text-gray-400">
               Welcome, <span className="text-[#f0b90b]">{user?.name?.split(' ')[0]}</span>
            </div>
            <NotificationSystem />
          </header>
        )}

        <div className={`flex-1 overflow-y-auto bg-[#0b0e11] ${token && !isHomePage ? 'pb-20 md:pb-6' : ''}`}>
          <Routes>
      ...
      {token && !isHomePage && (
        <nav className="fixed bottom-0 left-0 right-0 bg-[#161a1e]/95 backdrop-blur-md border-t border-[#1e2329] flex justify-around items-center py-3 md:hidden z-[100] shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
...
export default function App() {
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      ...
      } catch (e) { console.warn("Binance API connectivity issue"); }
    };
    fetchPrices();
    const timer = setInterval(fetchPrices, 12000);
