...
const AdminPanel = () => {
  ...
  useEffect(() => { fetchData(); }, [API_URL, token]);
  ...
  if (loading) return <div className="h-screen flex items-center justify-center text-yellow-500 font-black">LOADING...</div>;

  return (
    <div className="p-4 md:p-8 space-y-8 bg-[#0b0e11] min-h-screen text-left">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-black text-yellow-500 italic uppercase italic tracking-tighter">Command Center</h2>
        <div className="flex gap-2 bg-[#1e2329] p-1 rounded-xl border border-gray-800">
          <button onClick={() => setActiveTab('users')} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${activeTab === 'users' ? 'bg-yellow-500 text-black' : 'text-gray-500'}`}>Users</button>
          <button onClick={() => setActiveTab('requests')} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${activeTab === 'requests' ? 'bg-yellow-500 text-black' : 'text-gray-500'}`}>Requests</button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-[100] p-6 text-left">
          <div className="bg-[#1e2329] border border-gray-800 w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleBalanceUpdate} className="flex-1 bg-yellow-500 text-black py-4 rounded-2xl font-black uppercase text-[10px]">Save</button>
            </div>
          </div>
        </div>
      )}
