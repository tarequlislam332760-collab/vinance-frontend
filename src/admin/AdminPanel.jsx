...
/* ══════════════════════════════════════
   Cloudinary config — তোমার credentials
══════════════════════════════════════ */
const CLOUD_NAME    = 'dfe3wlx4u';
const UPLOAD_PRESET = 'trader_preset'; // Unsigned preset

/* ─── Cloudinary upload via fetch() ───
   axios ব্যবহার করলে Authorization header
   automatically যোগ হয় → Cloudinary reject করে।
   তাই শুধু fetch() ব্যবহার করতে হবে।
─────────────────────────────────────── */
async function uploadImage(file) {
  if (!file || !file.type.startsWith('image/')) throw new Error('Invalid file');
  if (file.size > 4 * 1024 * 1024) throw new Error('Max 4MB');
...
/* ────────────────────────────────────────────
   ImageUploader — Cloudinary only, no imgbb
──────────────────────────────────────────── */
const ImageUploader = ({ value, onChange, label = 'Profile Image' }) => {
  ...
  return (
    <div>
      ...
      <div
        style={{ border:`2px dashed ${drag?'#f0b90b':'#2b3139'}`, borderRadius:12, overflow:'hidden', position:'relative', minHeight:value?130:96, cursor:uploading?'wait':'pointer', background:drag?'rgba(240,185,11,.04)':'#0b0e11', transition:'all .2s', display:'flex', alignItems:'center', justifyContent:'center' }}
        ...
        {uploading ? (
          ...
          <>
            ...
            <button onClick={e => { e.stopPropagation(); onChange(''); }}
              style={{ position:'absolute', top:7, right:7, background:'rgba(0,0,0,.8)', border:'none', borderRadius:'50%', width:26, height:26, cursor:'pointer', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <X size={13}/>
            </button>
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:8, padding:20 }}>
            ...
            <span style={{ fontSize:11, color:'#5e6673' }}>PNG · JPG · WEBP — max 4MB</span>
          </div>
        )}
      </div>

      {error && (
        <div style={{ marginTop:6, fontSize:11, color:'#f6465d', display:'flex', alignItems:'center', gap:5 }}>
      ...
      {/* Cloudinary info */}
      <div style={{ marginTop:6, fontSize:10, color:'#5e6673' }}>
        Cloud: <code style={{ color:'#f0b90b' }}>{CLOUD_NAME}</code> · Preset: <code style={{ color:'#f0b90b' }}>{UPLOAD_PRESET}</code>
        {' '}(must be <span style={{ color:'#0ecb81' }}>Unsigned</span>)
      </div>
    </div>
  );
};
...
export default function AdminPanel() {
  ...
  const handleDelUser = async (id) => {
    ...
    try { await API.delete(`/api/admin/delete-user/${id}`); showToast('User deleted'); load(); }
    catch { showToast('Failed','err'); }
  };

  /* trader */
  const handleApprove = async (id) => {
    try { await API.put(`/api/admin/edit-trader/${id}`,{status:'approved'}); showToast('Approved!'); load(); }
    catch { showToast('Failed','err'); }
  };

  const handleDelTrader = async (id) => {
    if (!window.confirm('Delete this trader?')) return;
    try { await API.delete(`/api/admin/delete-trader/${id}`); showToast('Trader deleted'); load(); }
    catch { showToast('Failed','err'); }
  };
  ...
  const openEdit = (t) => {
    ...
    setEData({ name:safe(t.name), profit:t.profit||'', winRate:t.winRate||'', aum:t.aum||'', mdd:t.mdd||'', roi:t.roi||'', pnl:t.pnl||'', days:t.days||'', followers:t.followers||'', maxFollowers:t.maxFollowers||500, image:img, img, avatar:img });
    setEditModal(true);
  };
  ...
  const handleSaveTrader = async () => {
    ...
    if (img.startsWith('data:')) return showToast('Cannot save base64 image — upload via Cloudinary first','err');
    setSavingT(true);
    try {
      await API.put(`/api/admin/edit-trader/${selTrader._id}`, {
        ...eData, image:img, img, avatar:img,
      ...
      showToast('Trader updated!'); setEditModal(false); load();
  ...
  const handleSavePlan = async () => {
    ...
    try {
      ...
      showToast('Plan created!'); setPlanModal(false); setPlanData({}); load();
    } catch { showToast('Failed','err'); }
  ...
  const handleDelPlan = async (id) => {
    ...
    try { await API.delete(`/api/admin/delete-plan/${id}`); showToast('Plan deleted'); load(); }
    catch { showToast('Failed','err'); }
  };
  ...
  const filteredReqs = requests.filter(r => {
    ...
    const q = search.toLowerCase();
    return matchFilter&&(safe(r.userId?.name).toLowerCase().includes(q)||safe(r.type).toLowerCase().includes(q)||safe(r.userId?.email).toLowerCase().includes(q));
  });
    ...
    <div style={{ background:'#0b0e11', minHeight:'100vh', color:'#eaecef', fontFamily:"'Inter',sans-serif" }}>
      ...
      <div style={{ padding:'18px 22px' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10, background:'#161a1e', border:'1px solid #2b3139', borderRadius:10, padding:'8px 14px', marginBottom:16, maxWidth:380 }}>
          ...
          {search && <button onClick={()=>setSearch('')} style={{ background:'none', border:'none', color:'#848e9c', cursor:'pointer', padding:0 }}><X size={14}/></button>}
        </div>
        ...
        <div className="ns" style={{ display:'flex', gap:6, overflowX:'auto', marginBottom:18 }}>
          {TABS.map(t => (
            <button key={t.key} onClick={()=>{setTab(t.key);setSearch('');}}
              ...
              {t.badge!=null && <span style={{ background:tab===t.key?'rgba(0,0,0,.2)':'#2b3139', color:tab===t.key?'#0b0e11':'#eaecef', borderRadius:20, padding:'1px 6px', fontSize:10, fontWeight:700 }}>{t.badge}</span>}
            </button>
          ))}
        </div>
        ...
        {tab==='users' && (
          <div style={{ background:'#161a1e', borderRadius:18, border:'1px solid #1e2329', overflow:'hidden' }}>
            <div style={{ overflowX:'auto' }}>
              <table className="t">
                <thead><tr><th>User</th><th>Email</th><th>Role</th><th>Balance</th><th className="hs">Joined</th><th>Actions</th></tr></thead>
                <tbody>
                  {filteredUsers.length===0 && <tr><td colSpan={6} style={{ textAlign:'center', padding:40, color:'#5e6673' }}>No users found</td></tr>}
                  {filteredUsers.map(u => (
                    <tr key={u._id}>
                      <td>
                      ...
                      <td><span className="bd" style={{ background:u.role==='admin'?'rgba(240,185,11,.12)':'rgba(132,142,156,.1)', color:u.role==='admin'?'#f0b90b':'#848e9c' }}>{safe(u.role)}</span></td>
                      <td style={{ fontFamily:'monospace', fontWeight:700, color:'#eaecef' }}>{fmt(u.balance)}</td>
                      <td className="hs" style={{ color:'#848e9c', fontSize:12 }}>{u.createdAt?new Date(u.createdAt).toLocaleDateString():'-'}</td>
                      <td>
                        <div style={{ display:'flex', gap:6 }}>
                          <button className="btn b" style={{ padding:'5px 10px' }} onClick={()=>{setSelUser(u);setNewBal(String(u.balance||0));setBalModal(true);}}><Edit size={13}/> Balance</button>
                          <button className="btn r" style={{ padding:'5px 8px' }} onClick={()=>handleDelUser(u._id)}><Trash2 size={13}/></button>
                        </div>
                      </td>
                    </tr>
                  ))}
        ...
        {tab==='requests' && (
          <div>
            ...
            <div style={{ background:'#161a1e', borderRadius:18, border:'1px solid #1e2329', overflow:'hidden' }}>
              <div style={{ overflowX:'auto' }}>
                <table className="t">
                  <thead><tr><th>User</th><th>Type</th><th>Amount</th><th className="hs">Method</th><th className="hs">Date</th><th>Status</th><th>Actions</th></tr></thead>
                  <tbody>
                    {filteredReqs.length===0 && <tr><td colSpan={7} style={{ textAlign:'center', padding:40, color:'#5e6673' }}>No requests found</td></tr>}
                    {filteredReqs.map(r => (
                      <tr key={r._id}>
                        <td>
                        ...
                        <td><span style={{ display:'flex', alignItems:'center', gap:5, fontWeight:600, color:r.type==='deposit'?'#0ecb81':'#f6465d', fontSize:12 }}>{r.type==='deposit'?<ArrowDownLeft size={13}/>:<ArrowUpRight size={13}/>}{safe(r.type)}</span></td>
                        <td style={{ fontFamily:'monospace', fontWeight:700, color:'#eaecef' }}>{fmt(r.amount)}</td>
                        <td className="hs" style={{ color:'#848e9c', fontSize:12 }}>{safe(r.method||r.address)||'-'}</td>
                        <td className="hs" style={{ color:'#848e9c', fontSize:12 }}>{r.createdAt?new Date(r.createdAt).toLocaleDateString():'-'}</td>
                        <td><span className="bd" style={{ background:r.status==='approved'||r.status==='completed'?'rgba(14,203,129,.12)':r.status==='rejected'?'rgba(246,70,93,.12)':'rgba(240,185,11,.12)', color:r.status==='approved'||r.status==='completed'?'#0ecb81':r.status==='rejected'?'#f6465d':'#f0b90b' }}>{safe(r.status)||'pending'}</span></td>
                        <td>{r.status==='pending'&&<div style={{ display:'flex', gap:5 }}><button className="btn g" style={{ padding:'5px 8px' }} onClick={()=>handleRequest(r._id,'approved')}><CheckCircle size={13}/></button><button className="btn r" style={{ padding:'5px 8px' }} onClick={()=>handleRequest(r._id,'rejected')}><X size={13}/></button></div>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                    ...
                    <div key={t._id} style={{ background:'#161a1e', border:'1px dashed #2b3139', borderRadius:13, padding:15, display:'flex', justifyContent:'space-between', alignItems:'center', gap:10 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:9, minWidth:0 }}>
                        <div style={{ width:40, height:40, borderRadius:'50%', background:'#2b3139', overflow:'hidden', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:15, fontWeight:800 }}>
                          {t.image||t.img||t.avatar ? <img src={safe(t.image||t.img||t.avatar)} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} onError={e=>e.target.style.display='none'}/> : safe(t.name)[0]?.toUpperCase()||'T'}
                        </div>
                        <div>
                          <p style={{ fontWeight:700, color:'#eaecef', fontSize:13 }}>{safe(t.name)}</p>
                          <p style={{ color:'#848e9c', fontSize:11 }}>ROI: {t.roi||t.profit||0}%</p>
                        </div>
                      </div>
        ...
        {tab==='plans' && (
          <div>
            <div style={{ display:'flex', justifyContent:'flex-end', marginBottom:14 }}>
              <button className="btn y" onClick={()=>{setPlanData({});setPlanModal(true);}}><UserPlus size={14}/> Add Plan</button>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))', gap:13 }}>
              ...
              {plans.length===0 && <div style={{ gridColumn:'1/-1', textAlign:'center', padding:40, color:'#5e6673' }}><PieChart size={36} style={{ opacity:.15, margin:'0 auto 12px', display:'block' }}/><p>No plans yet.</p></div>}
            </div>
          </div>
        )}
        ...
        {tab==='logs' && (
          <div style={{ background:'#161a1e', borderRadius:18, border:'1px solid #1e2329', overflow:'hidden' }}>
            <div style={{ overflowX:'auto' }}>
              <table className="t">
                <thead><tr><th>User</th><th>Plan</th><th>Amount</th><th>Profit</th><th>Status</th><th className="hs">Expires</th></tr></thead>
                <tbody>
                  {investments.length===0 && <tr><td colSpan={6} style={{ textAlign:'center', padding:40, color:'#5e6673' }}>No investments found</td></tr>}
                  {investments.map((inv,i) => (
                    <tr key={inv._id||i}>
                      ...
                      <td><span className="bd" style={{ background:inv.status==='completed'?'rgba(14,203,129,.12)':'rgba(240,185,11,.12)', color:inv.status==='completed'?'#0ecb81':'#f0b90b' }}>{safe(inv.status)}</span></td>
                      <td className="hs" style={{ color:'#848e9c', fontSize:12 }}>{inv.expireAt?new Date(inv.expireAt).toLocaleString():'-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {balModal && (
        <div className="mb" onClick={e=>e.target===e.currentTarget&&setBalModal(false)}>
          <div className="mx">
            ...
            <p style={{ fontSize:12, color:'#848e9c', marginBottom:10 }}>Current: <span style={{ color:'#f0b90b', fontWeight:700 }}>{fmt(selUser?.balance)}</span></p>
            <div style={{ position:'relative', marginBottom:20 }}>
              <span style={{ position:'absolute', left:14, top:'50%', transform:'translateY(-50%)', color:'#f0b90b', fontWeight:700, fontSize:16 }}>$</span>
              <input className="ai" type="number" value={newBal} onChange={e=>setNewBal(e.target.value)} placeholder="0.00" style={{ paddingLeft:32, fontSize:20, fontWeight:800, fontFamily:'monospace' }}/>
            </div>
          ...
          <div className="mx">
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              <div>
                ...
                <input className="ai" placeholder="Trader name" value={safe(eData.name)} onChange={e=>setEData(p=>({...p,name:e.target.value}))}/>
              </div>
              <ImageUploader
                label="Profile Photo"
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:11 }}>
                {[
                  {k:'roi',l:'ROI (%)',p:'45'},{k:'pnl',l:'PNL ($)',p:'12500'},
                  {k:'profit',l:'Profit (%)',p:'38'},{k:'winRate',l:'Win Rate (%)',p:'78'},
                  {k:'aum',l:'AUM ($)',p:'250000'},{k:'mdd',l:'Drawdown (%)',p:'12'},
                  {k:'days',l:'Days Active',p:'120'},{k:'followers',l:'Followers',p:'320'},
                  ...
                  <div key={f.k}>
                    ...
                    <input className="ai" type="number" placeholder={f.p} value={eData[f.k]!=null?String(eData[f.k]):''} onChange={e=>setEData(p=>({...p,[f.k]:e.target.value}))} style={{ padding:'8px 11px', fontSize:13 }}/>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:13 }}>
              {[
                ...
                <div key={f.k}>
                  ...
                  <input className="ai" type={f.t} placeholder={f.p} value={safe(planData[f.k])} onChange={e=>setPlanData(p=>({...p,[f.k]:e.target.value}))}/>
                </div>
              ))}
            </div>
            <div style={{ display:'flex', gap:10, marginTop:22 }}>
...
/* ══════════════════════════════════════════════
   AddTraderInline — Cloudinary only, no imgbb
══════════════════════════════════════════════ */
function AddTraderInline({ onSuccess }) {
  ...
  const handleFile = async (file) => {
    ...
    setUploading(true);
    try {
      const url = await uploadImage(file);  /* Cloudinary fetch — no axios */
  ...
  const handleSubmit = async () => {
    const name = safe(form.name).trim();
    if (!name) return alert('Name required');
    if (form.image.startsWith('data:')) return alert('Please upload via Cloudinary — base64 is too large for the server (413 error).');
    setSaving(true);
    try {
      await API.post('/api/admin/create-trader', {
        name, image:form.image, img:form.image, avatar:form.image,
      ...
      const msg = safe(e?.response?.data?.message)||'Failed';
      if (e?.response?.status===413) alert('413: Payload too large. Use Cloudinary URL — not base64 image.');
      else alert(msg);
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
        <div style={{ width:56, height:56, borderRadius:'50%', background:'#2b3139', overflow:'hidden', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', border:'2px dashed #3a3f47', position:'relative' }}
          onClick={()=>!uploading&&inputRef.current?.click()}>
          {uploading
            ? <Loader2 size={18} style={{ color:'#f0b90b', animation:'spin .8s linear infinite' }}/>
        ...
        <div style={{ flex:1 }}>
          <button
            ...
            style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 14px', background:'rgba(240,185,11,.1)', border:'1px solid rgba(240,185,11,.3)', borderRadius:8, color:'#f0b90b', fontWeight:700, fontSize:12, cursor:uploading?'not-allowed':'pointer', fontFamily:'inherit', marginBottom:6 }}>
            {uploading?<><Loader2 size={12} style={{ animation:'spin .8s linear infinite' }}/> Uploading...</>:<><Upload size={12}/> Upload Image</>}
          </button>
          <input style={{ width:'100%', background:'#0b0e11', border:'1px solid #2b3139', borderRadius:8, padding:'7px 11px', color:'#eaecef', fontSize:12, outline:'none', fontFamily:'inherit', transition:'border .15s' }}
            placeholder="Or paste Cloudinary/image URL"
            value={form.image&&!form.image.startsWith('data:')?form.image:''}
            onChange={e=>setForm(p=>({...p,image:e.target.value}))}
            onFocus={e=>e.target.style.borderColor='#f0b90b'}
            onBlur={e=>e.target.style.borderColor='#2b3139'}/>
          {upErr && <p style={{ fontSize:10, color:'#f6465d', marginTop:3 }}>{upErr}</p>}
          <p style={{ fontSize:10, color:'#5e6673', marginTop:3 }}>
            Cloud: <code style={{ color:'#f0b90b' }}>{CLOUD_NAME}</code> · Preset: <code style={{ color:'#f0b90b' }}>{UPLOAD_PRESET}</code>
          </p>
        </div>
      </div>
        ...
        {[
          ...
          <div key={f.k}>
            ...
            <input style={{ width:'100%', background:'#0b0e11', border:'1px solid #2b3139', borderRadius:8, padding:'8px 11px', color:'#eaecef', fontSize:13, outline:'none', fontFamily:'inherit', transition:'border .15s' }}
              type={f.t} placeholder={f.p} value={safe_(form[f.k])}
      ...
      <button onClick={handleSubmit} disabled={saving||uploading||!safe_(form.name).trim()}
        style={{ padding:'10px 28px', background:(saving||uploading||!safe_(form.name).trim())?'#2b3139':'#f0b90b', border:'none', borderRadius:10, color:(saving||uploading||!safe_(form.name).trim())?'#5e6673':'#0b0e11', fontWeight:700, fontSize:13, cursor:(saving||uploading||!safe_(form.name).trim())?'not-allowed':'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:7 }}>
